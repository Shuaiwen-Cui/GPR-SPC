...
# def trial(stiffness_drop, noise_level):
def trial(stiffness_drop):
    # ensure reduction is between 0 and 1
    if stiffness_drop < 0 or stiffness_drop > 1:
        raise ValueError("Stiffiness drop must be between 0 and 1")
    ...
    height_pier = 7.0  # Height of the bridge piers (m)
    num_span = 3  # Number of spans
    ...
    ops.recorder('Node', '-file', 'pa.txt', '-time', '-node', 3, '-dof', 1, 2, 3, 'disp')
    ops.recorder('Node', '-file', 'pb.txt', '-time', '-node', 6, '-dof', 1, 2, 3, 'disp')
    ...
    point_A = np.loadtxt('pa.txt')
    point_B = np.loadtxt('pb.txt')
    ...
    # disp_XA_noisy = disp_XA + noise_A
    # disp_XB_noisy = disp_XB + noise_B
    
    # disp_YA_noisy = disp_YA + noise_A
    # disp_YB_noisy = disp_YB + noise_B
    
    # disp_ZA_noisy = disp_ZA + noise_A
    # disp_ZB_noisy = disp_ZB + noise_B
    
    # max_XA_noisy = np.max(np.abs(disp_XA_noisy))
    # max_XB_noisy = np.max(np.abs(disp_XB_noisy))
    
    # max_YA_noisy = np.max(np.abs(disp_YA_noisy))
    # max_YB_noisy = np.max(np.abs(disp_YB_noisy))
    
    # max_ZA_noisy = np.max(np.abs(disp_ZA_noisy))
    # max_ZB_noisy = np.max(np.abs(disp_ZB_noisy))
    
    # return max_XA, max_XB, max_YA, max_YB, max_ZA, max_ZB, max_XA_noisy, max_XB_noisy, max_YA_noisy, max_YB_noisy, max_ZA_noisy, max_ZB_noisy
